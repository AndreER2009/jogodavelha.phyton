...
def checar_vitoria(elemento: list) -> bool:
    ...
    for i in range(len(elemento) - 2):
        ...
        if (elemento[i] == elemento[i + 1] == elemento[i + 2] or 
            elemento[i - 1] == elemento[i] == elemento[i + 1] or
                elemento[i - 2] == elemento[i - 1] == elemento[i]) and elemento[i] != " ":
            return True

    for i in range(len(elemento) - 6):
        # Vitória na vertical
        if (elemento[i] == elemento[i + 3] == elemento[i + 6] or 
            elemento[i - 3] == elemento[i] == elemento[i + 3] or
